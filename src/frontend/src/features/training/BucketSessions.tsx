...
export function BucketSessions({ groupId, start, end }: { groupId: number; start: string; end: string }) {
  ...
  return <table aria-label={t("contributingSessions")}><thead><tr><th>{t("date")}</th><th>{t("polarType")}</th><th>{t("duration")}</th><th>{t("pace")}</th><th>{t("heartRate")}</th><th>{t("distancePaceIndex")}</th></tr></thead><tbody>{query.data.map((session) => <tr key={session.external_id}><td>{session.local_date}</td><td>{session.sport_type}</td><td>{session.duration_seconds === null ? "-" : t("minuteValue", { value: Math.round(session.duration_seconds / 60) })}</td><td>{session.average_pace_seconds_per_kilometer === null ? "-" : t("paceValue", { value: `${Math.floor(session.average_pace_seconds_per_kilometer / 60)}:${String(Math.round(session.average_pace_seconds_per_kilometer % 60)).padStart(2, "0")}` })}</td><td>{session.average_heart_rate ?? "-"}</td><td>{session.distance_pace_index?.toFixed(3) ?? "-"}</td></tr>)}</tbody></table>;
}
